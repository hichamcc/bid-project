...
<div class="py-12">
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Other General Contractors</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($project->other_gc as $gcName => $gcData)
                        <div class="bg-gray-50 p-4 rounded-lg border">
                            <h4 class="font-semibold text-gray-900 mb-3">{{ $gcName }}</h4>
                            <div class="space-y-2">
                                @if(isset($gcData['due_date']) && $gcData['due_date'])
                                <div>
                                    <label class="block text-sm font-medium text-gray-500">Due Date</label>
                                    <p class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($gcData['due_date'])->format('M d, Y') }}</p>
                                </div>
                                @endif
                                
                                @if(isset($gcData['web_link']) && $gcData['web_link'])
                                <div>
                                    <label class="block text-sm font-medium text-gray-500">Web Link</label>
                                    <a href="{{ $gcData['web_link'] }}" 
                                       target="_blank" 
                                       class="inline-flex items-center text-blue-600 hover:text-blue-800 text-sm break-all">
                                ...
                                @if((!isset($gcData['due_date']) || !$gcData['due_date']) && (!isset($gcData['web_link']) || !$gcData['web_link']))
                                <p class="text-sm text-gray-500">No additional details</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
