        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-semibold text-gray-900">GC Management</h2>
                    <a href="{{ route('admin.gcs.create') }}" 
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="overflow-x-auto">
                @php
                    ...
                    // Build a sort link toggling asc/desc, preserving other query params.
                    $gcSortLink = function (string $key) use ($currentSort, $currentDir) {
                        $isActive = $currentSort === $key;
                        $nextDir  = ($isActive && $currentDir === 'desc') ? 'asc' : 'desc';
                        $q = array_merge(request()->query(), ['sort' => $key, 'direction' => $nextDir]);
                        unset($q['page']);
                        return ['url' => route('admin.gcs.index') . '?' . http_build_query($q), 'active' => $isActive, 'dir' => $currentDir];
                    };
                @endphp
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            @php $muH = $gcSortLink('mu'); @endphp
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ $muH['url'] }}" class="group inline-flex items-center gap-1 hover:text-gray-700">
                                    <span>MU</span>
                                    @if($muH['active'])
                                        <span class="text-gray-700">{{ $muH['dir'] === 'asc' ? '▲' : '▼' }}</span>
                                    @else
                                        <span class="text-gray-300 group-hover:text-gray-400">↕</span>
                                    @endif
                                </a>
                            </th>
                            @php $nonMuH = $gcSortLink('non_mu'); @endphp
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ $nonMuH['url'] }}" class="group inline-flex items-center gap-1 hover:text-gray-700">
                                    <span>NON MU</span>
                                    @if($nonMuH['active'])
                                        <span class="text-gray-700">{{ $nonMuH['dir'] === 'asc' ? '▲' : '▼' }}</span>
                                    @else
                                        <span class="text-gray-300 group-hover:text-gray-400">↕</span>
                                    @endif
                                </a>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-xs text-gray-400">{{ $gc->created_at->diffForHumans() }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <form method="POST" 
                                              ...
                                              class="inline"
                                              onsubmit="return confirm('Are you sure you want to delete this GC?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
